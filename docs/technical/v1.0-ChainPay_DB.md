...
CREATE TABLE employees (
    ...
    auto_invest_vault_id VARCHAR(100),
    auto_invest_type    VARCHAR(10),
    auto_invest_value   NUMERIC(18, 6),
...
| cron_enabled | BOOLEAN | 是否开启自动发薪（默认 true，需全局开关也开启才生效）|
| auto_invest_enabled | BOOLEAN | 是否开启自动定投理财（默认 false）|
| auto_invest_vault_id | VARCHAR(100) | LiFi Earn Vault ID |
| auto_invest_type | VARCHAR(10) | 定投类型：`percentage`（基点，如 1000=10%）或 `fixed`（固定 USDC 金额）|
| auto_invest_value | NUMERIC(18,6) | 定投数值（基点或 USDC 金额）|
...
db.Where(
    "next_pay_date <= ? AND has_rules = ? AND cron_enabled = ?",
    time.Now().UTC(), true,
).Find(&employees)
```
